...
void Scanner::Init() {
	...
	for (i = 13; i <= 13; ++i) start.set(i, 7);
	for (i = 0; i <= 9; ++i) start.set(i, 9);
	for (i = 11; i <= 12; ++i) start.set(i, 9);
	for (i = 14; i <= 34; ++i) start.set(i, 9);
	for (i = 36; i <= 36; ++i) start.set(i, 9);
	for (i = 38; i <= 39; ++i) start.set(i, 9);
	for (i = 44; i <= 44; ++i) start.set(i, 9);
	for (i = 58; i <= 60; ++i) start.set(i, 9);
	for (i = 62; i <= 90; ++i) start.set(i, 9);
	for (i = 92; i <= 92; ++i) start.set(i, 9);
	for (i = 96; i <= 96; ++i) start.set(i, 9);
	for (i = 123; i <= 65535; ++i) start.set(i, 9);
	start.set(46, 4);
	start.set(35, 10);
	start.set(40, 11);
	start.set(37, 12);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 1:
			...
			if (ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_1;}
		...
		case 6:
			...
			if (ch == 13) {AddCh(); goto case_8;}
			else {t->kind = 4; break;}
		case 7:
			recEnd = pos; recKind = 4;
			if (ch == 10) {AddCh(); goto case_8;}
			else {t->kind = 4; break;}
		case 8:
			case_8:
			{t->kind = 4; break;}
		case 9:
			...
			if (ch <= 9 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= L'$') || (ch >= L'&' && ch <= 39) || ch == L',' || (ch >= L':' && ch <= L'<') || (ch >= L'>' && ch <= L'Z') || ch == 92 || ch == L'`' || (ch >= L'{' && ch <= 65535)) {AddCh(); goto case_9;}
			else {t->kind = 5; break;}
		case 10:
			recEnd = pos; recKind = 5;
			if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_5;}
			else if (ch <= 9 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= L'$') || (ch >= L'&' && ch <= 39) || ch == L',' || (ch >= L':' && ch <= L'<') || (ch >= L'>' && ch <= L'Z') || ch == 92 || ch == L'`' || (ch >= L'{' && ch <= 65535)) {AddCh(); goto case_9;}
			else {t->kind = 5; break;}
		case 11:
			{t->kind = 6; break;}
		case 12:
